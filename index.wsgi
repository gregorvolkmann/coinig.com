...
def application(environ, start_response):
    ...
    for key in url_map:
        if key and request.path.startswith(key):
            return static(request, url_map[key])

    data = {"adr": "", "msg": "", "sig": ""}
    if request.method == 'GET':
        data["adr"] = escape(request.args.get("adr", "").encode("ascii", "ignore"))
        data["msg"] = escape(request.args.get("msg", "").encode("ascii", "ignore"))
        data["sig"] = escape(request.args.get("sig", "").encode("ascii", "ignore"))
    elif request.method == 'POST':
        data["adr"] = escape(request.form["adr"].encode("ascii", "ignore"))
        data["msg"] = escape(request.form["msg"].encode("ascii", "ignore"))
        data["sig"] = escape(request.form["sig"].encode("ascii", "ignore"))
    data["res"] = ""
    
    page = file("page.tpl").read()
    title = "Bitcoin signature verification"
    form = file("form.tpl").read()

    success = "<div class=\"alert alert-success\"><strong>Verified</strong></div>"
    failure = "<div class=\"alert alert-danger\"><strong>Failure:</strong> %s</div>"

    if data["adr"] != "" and data["msg"] != "" and data["sig"] != "":
        try:
            verification = verifySignature(data["adr"], data["sig"], data["msg"])
            data["res"] = success
        except Exception as e:
            data["res"] = failure % str(e)

    content = form % data

    c = {"title": title, "content": content}

    response = Response(page % c, content_type="text/html")
    response.status = "200 OK"
    response.status_code = 200

    return response(environ, start_response)

def static(request, content_type):
    base = os.path.dirname(__file__)
    asset_fname = os.path.join(base, request.path[1:])
    asset = file(asset_fname).read()
    return Response(asset, content_type=content_type)
